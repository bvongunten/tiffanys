package ch.nostromo.tiffanys.commons.opening.polyglot;

import ch.nostromo.tiffanys.commons.exception.PolyglotException;
import ch.nostromo.tiffanys.commons.format.FenFormat;
import ch.nostromo.tiffanys.commons.opening.OpeningBookMove;
import ch.nostromo.tiffanys.commons.opening.OpeningBookMoveSet;
...
public class Polyglot {
     ...
     */
    public static Map<Long, OpeningBookMoveSet> loadOpeningBook(String filename) {
        try {

            Map<Long, OpeningBookMoveSet> moveSets = new HashMap<>();

            BufferedInputStream infile = new BufferedInputStream(Objects.requireNonNull(Polyglot.class.getResourceAsStream(filename)));

            // Process each move in the book
            byte[] buffer = new byte[MOVE_SIZE];
            while (infile.read(buffer, 0, MOVE_SIZE) != -1) {
                ByteBuffer byteBuffer = ByteBuffer.wrap(buffer);

                long position = byteBuffer.getLong();

                short move = byteBuffer.getShort();
                ...
                // illegal move
                if (toFile == 0 && toRank == 0 && fromFile == 0 && fromRank == 0) {
                    continue;
                }

                OpeningBookMove openingBookMove = new OpeningBookMove(move, weight, fromRank, fromFile, toRank, toFile, promotionPiece);

                moveSets.putIfAbsent(position, new OpeningBookMoveSet());

                moveSets.get(position).addMove(openingBookMove);
            }

            return moveSets;

        } catch (IOException e) {
            throw new PolyglotException("Unable to load polyglot opening book: " + filename, e);
        }

    }

    public static long getPolyglotZobristHash(FenFormat fenFormat) {
        String[] fenParts = fenFormat.toString().split(" ");
        String pieces = fenParts[0];
        String turn = fenParts[1];
        String castling = fenParts[2];
        String epField = fenParts[3];

        char[][] board = parseBoard(pieces);
        long hash = computePieceHash(board);

        hash ^= computeTurnHash(turn);
        hash ^= computeCastlingHash(castling);
        hash ^= computeEnPassantHash(epField, turn, board);

        return hash;
    }

    private static char[][] parseBoard(String pieces) {
    ...
    private static long computePieceHash(char[][] board) {
        long hash = 0;
        ...
        for (int rank = 0; rank < BOARD_HEIGHT; rank++) {
            for (int file = 0; file < BOARD_WIDTH; file++) {
                ...
                int pieceIndex = PIECES.indexOf(p);
                int offset = NUM_SQUARES * pieceIndex + rank * BOARD_WIDTH + file;

                hash ^= POLYGLOT_HASHES[offset];
            }
        }
        return hash;
    }
    private static long computeTurnHash(String turn) {
        return turn.charAt(0) == 'w'
                ? POLYGLOT_HASHES[TURN_OFFSET]
                : 0;
    }

    private static final Map<Character, Integer> CASTLE_MAP = Map.of(
            'K', 0,
            'Q', 1,
            'k', 2,
            'q', 3
    ...
    private static long computeCastlingHash(String castling) {
        if (castling.equals("-")) return 0;

        long hash = 0;
        for (int i = 0; i < castling.length(); i++) {
            Integer off = CASTLE_MAP.get(castling.charAt(i));
            if (off != null) {
                hash ^= POLYGLOT_HASHES[CASTLE_OFFSET + off];
            }
        }
        return hash;
    }
    ...
    private static long computeEnPassantHash(String epField, String turn, char[][] board) {
        if (epField.charAt(0) == '-') return 0;

        int epFile = epField.charAt(0) - 'a';
        boolean white = turn.charAt(0) == 'w';

        int rank = white ? 4 : 3;
        char pawn = white ? 'P' : 'p';

        boolean left = epFile > 0     && board[rank][epFile - 1] == pawn;
        boolean right = epFile < 7    && board[rank][epFile + 1] == pawn;

        return (left || right)
                ? POLYGLOT_HASHES[EN_PASSANT_OFFSET + epFile]
                : 0;
    }
